...
async def async_setup_entry(
    ...
    if device_type in ["leak_detector"] or device_type == 4:
        ...
        _LOGGER.info("Created binary sensor entity for leak detector")
...
class GemnsBLESensor(SensorEntity):
    ...
    def _set_sensor_properties(self) -> None:
        ...
        # Skip leak detectors - they should be handled by binary sensor
        if "leak" in device_type:
            # Don't create sensor entities for leak detectors
            return
    ...
    def _extract_sensor_value(self, data: Dict[str, Any]) -> None:
        ...
        # Skip leak detectors - they should be handled by binary sensor
        if "leak_detected" in sensor_data:
            # Don't process leak detectors in regular sensor
            _LOGGER.info("LEAK DETECTOR SKIPPED: %s | Leak detected: %s (handled by binary sensor)", 
