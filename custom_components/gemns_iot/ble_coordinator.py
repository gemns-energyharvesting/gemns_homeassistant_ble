...
class GemnsBluetoothProcessorCoordinator(
    ...
    async def async_init(self) -> None:
        ...
        # For event-driven devices (like leak detectors), don't require immediate advertisement
    ...
    def _parse_advertisement_data(self, service_info: BluetoothServiceInfo) -> dict[str, Any]:
        ...
        if 'sensor_data' in data and 'device_type' in data['sensor_data']:
            ...
            if device_type == 1:
                ...
                data["device_type"] = "leak_detector"
                data["name"] = f"Gemns™ IoT Leak Detector {professional_id}"
                _LOGGER.info("  Identified as: leak_detector")
            elif device_type == 5:
                data["device_type"] = "vibration_sensor"
        ...
        try:
            ...
            test_data = {
                ...
                "decrypted_data": {
                    ...
                    "device_type": b'\x00\x04',  # Leak detector
